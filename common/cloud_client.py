...
通过LinkAI套接字协议处理远程配置同步、消息推送和技能管理。

注意：默认情况下，云相关配置是禁用的。应用程序完全在本地运行，
不会连接任何远程服务。只有满足以下两个条件时，云客户端才会被激活：

  1. config中设置了 ``use_linkai`` 为True（在app.py中导入此模块前检查）。
  2. ``cloud_deployment_id``（或环境变量CLOUD_DEPLOYMENT_ID）非空
     （在app.py和下面的 ``start()`` 函数中检查）。

如果缺少任一条件，此模块将不会被加载，程序将继续作为纯本地应用程序运行。
"""

"""
Agent Initializer - 处理Agent初始化逻辑
"""
...
class AgentInitializer:
    ...
    def __init__(self, bridge, agent_bridge):
        ...
        参数:
            bridge: COW桥接实例
            agent_bridge: AgentBridge实例（用于create_agent方法）
        """
        self.bridge = bridge
    ...
    def initialize_agent(self, session_id: Optional[str] = None) -> Agent:
        ...
        agent = self.agent_bridge.create_agent(
            ...
            runtime_info=runtime_info  # 传递runtime_info用于动态时间更新
        )
        ...
        # 启动每日记忆刷新定时器（在首次Agent初始化时启动一次，无论会话如何）
